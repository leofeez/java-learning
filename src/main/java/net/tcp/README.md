...
为什么四次挥手后，发起断开连接的一方会进入TIME_WAIT 2MSL?

因为最后一次挥手的ACK如果因为网络故障没有到达对端，那么对端还会有可能重发FIN，如果发起端在最后一次ACK直接closed，那么最后一次的FIN
就没办法收到，最终会导致对端无法正常进入closed状态。在TIME_WAIT状态也会消耗资源，主要消耗的是socket四元组。
